...
CScene7::CScene7()
{
	...
	pTextures->CreateTextureMipMap(0, "../Scene7/CasaTex.jpg");


	LoadModel("../Scene7/Casa.txt");

	for (int i = 0; i < 38; i++)
		houseColor[i] = { (rand() % 255), (rand() % 255), (rand() % 255) };
...
void CScene7::LoadModel(const string& filename) {
	...
	while (getline(arq, line)) {
		...
		if (line.size() != 0) {
			if (line.size() <= 11) {
				if (istring >> c >> texture.x >> c >> texture.y) {
					textures.push_back(texture);
				}
			}
			else {
				if (istring >> vertex.x >> c >> vertex.y >> c >> vertex.z) {
